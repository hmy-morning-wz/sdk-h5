...
class CPService {
	...
	async getCurrentTrip() {
		return Get('/h5/api/trip', {cityId: this.cityId}, this.config);
	}

	// 查询无锡地铁站点
	async loadStations() {
		return Get('/h5/api/station', {cityId: this.cityId}, this.config);
	}

	//执行补登操作
	async claim(claimData) {
		return Post('/h5/api/claim', claimData, this.config);
	}
