				...
				<div>
					<List>
						<List.Item 
							...
							arrow="horizontal" 
							extra={renderPayAmt(rowData)} 
							onClick={detailInfo(rowData)}>
							<span className={styles.payTip}>充值</span> 
							<List.Item.Brief><span style={{fontSize: '0.24rem', color: '#A2A2A2',fontFamily: 'PingFangSC-Regular'}}> {rowData.createTime} </span></List.Item.Brief>
						</List.Item>
					</List>
				</div>
			);
		...
		const renderPayAmt = (rowData) => {
			switch (rowData.status) {
				case PayStatus.SUCCESS:
				  return (
					<div className={styles.extraBan}>					  
					  <span className={styles.payAmt}>{Utils.formatRMBYuan(rowData.rechargeAmt)}  元</span>
			...
			<DocumentTitle title="充值记录">				
				<div>
					...
					<div className={styles.contain}>
							...
							>
								{
									(this.state.itemNum === 0 && !this.state.isLoading) ? <div className={styles.nodata}><img src={`${global.csConf.cdnHost}/images/610300/recordnone2.png`} alt="No Data" /><p>暂时还没有内容哦~</p></div> : ''
								}
							</ListView>
						</div>
					</Loading>
